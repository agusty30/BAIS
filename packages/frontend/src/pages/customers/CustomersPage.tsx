...
const CUSTOMER_CATEGORIES = ['Individual', 'Corporate', 'Government', 'Non-Profit', 'Other'];
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {isLoading ? (
              ...
              <tr><td colSpan={7} className="px-6 py-8 text-center text-slate-500 dark:text-slate-400">No customers found</td></tr>
            ) : (
              (data?.data || []).map((c: any) => (
                <tr key={c.id} className="table-row">
                  <td className="px-6 py-3 text-sm font-medium text-slate-900 dark:text-white">{c.name}</td>
                  <td className="px-6 py-3 text-sm text-slate-600 dark:text-slate-300">{c.category || '-'}</td>
...
function CreateCustomerModal({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = useState({ name: '', email: '', phone: '', address: '', taxId: '', category: '', creditLimit: 0 });
